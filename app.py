...
POST_SLOTS_PER_RUN = int(os.getenv("POST_SLOTS_PER_RUN", "3"))
RSS_LIST = [
...
def call_openai(prompt: str) -> str:
    ...
    for attempt in range(3):
        ...
        if r.status_code == 429:
            ...
            print(f"[OpenAI] 429 Too Many Requests → {wait_sec}秒待って再試行します")
            time.sleep(wait_sec)
            continue
        r.raise_for_status()
        return r.json()["choices"][0]["message"]["content"].strip()

    # 3回とも429だった場合は最後のレスポンスをエラーにする
    r.raise_for_status()
    return ""
